...
export class LocalFloraTrainer {
	...
	static deserialize(
		...
		const trainer = new LocalFloraTrainer(mapId)
	...
	static async load(mapId: string): Promise<LocalFloraTrainer | null> {
		const db = await openDB()
		return new Promise((resolve, reject) => {
			...
			req.onsuccess = () => {
				db.close()
				if (req.result) {
					resolve(LocalFloraTrainer.deserialize(mapId, req.result))
				} else {
					resolve(null)
				}
			}
...
function openDB(): Promise<IDBDatabase> {
	return new Promise((resolve, reject) => {
		const req = indexedDB.open('flora', 1)
		req.onupgradeneeded = () => {
			const db = req.result
