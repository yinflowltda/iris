...
import { PrismaEmbeddingService } from '../../../client/lib/prisma/embedding-service'
import type { PrismaWorkerResponse } from '../../../client/lib/prisma/types'
...
class MockWorker {
	...
	simulateMessage(data: PrismaWorkerResponse) {
...
describe('PrismaEmbeddingService', () => {
	beforeEach(() => {
		mockWorker = new MockWorker()
		PrismaEmbeddingService._resetInstance()
	})
	...
	it('returns the same singleton instance', () => {
		const a = PrismaEmbeddingService.getInstance()
		const b = PrismaEmbeddingService.getInstance()
		expect(a).toBe(b)
	})
	...
	it('isSupported() checks for Worker and WebAssembly', () => {
		expect(PrismaEmbeddingService.isSupported()).toBe(true)
	})
	...
	it('dispose() terminates worker and cleans up', async () => {
		...
		const newInstance = PrismaEmbeddingService.getInstance()
