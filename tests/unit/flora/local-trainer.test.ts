...
import { contrastiveLoss, LocalFloraTrainer } from '../../../client/lib/flora/local-trainer'
import { dot } from '../../../client/lib/flora/projection-head'
...
describe('LocalFloraTrainer', () => {
	it('starts uninitialized with zero examples', () => {
		const trainer = new LocalFloraTrainer('test-map')
	...
	it('serialize/deserialize roundtrips', () => {
		...
		const restored = LocalFloraTrainer.deserialize('test-map', serialized)
	...
	it('training improves classification accuracy', async () => {
		...
		const trainer = new LocalFloraTrainer('test')
