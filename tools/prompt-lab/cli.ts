...
async function main(): Promise<void> {
	...
	const initialPrompt = await buildSystemPromptForFramework(framework)
	...
	const report = await runLoop({
		...
		onProgress: async (iter, total, result) => {
			const rollback = result.rolledBack ? ' (rollback)' : ''
			const status = result.accepted ? '+' : 'x'
			console.log(
				`  [${iter}/${total}] ${status} avg=${result.averageOverall.toFixed(1)}/10${rollback}`,
			)
		},
	})
	...
	// Save best prompt to disk
	const bestPromptPath = join('results', report.runId, 'best-prompt.txt')
	await writeFile(join(import.meta.dirname, bestPromptPath), report.bestPrompt, 'utf-8')

	// Print summary
	const changeStr = `${report.improvement >= 0 ? '+' : ''}${report.improvement.toFixed(1)}`
	console.log('')
	console.log(`  Run ID:    ${report.runId}`)
	console.log(`  Baseline:  ${report.baselineAverage.toFixed(1)}/10`)
	console.log(`  Best:      ${report.bestAverage.toFixed(1)}/10`)
	...
	console.log(`  Best prompt: tools/prompt-lab/results/${report.runId}/best-prompt.txt`)
	console.log('')
}
